...
class _AdminProductScreenState extends State<AdminProductScreen> {
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      final response = await http.get(Uri.parse('http://192.168.54.252:5274/api/product'));
  ...
  Future<void> _addProduct() async {
    ...
    final response = await http.post(
      Uri.parse('http://192.168.54.252:5274/api/product'),
  ...
  Future<void> _deleteProduct(int id) async {
    final response = await http.delete(Uri.parse('http://192.168.54.252:5274/api/product/$id'));
    if (response.statusCode == 200) {
      await _loadProducts();
    }
  }
  ...
  Widget _buildProductPage() {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            const Text("Ajouter un produit", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            ...
            const SizedBox(height: 8),
            TextField(controller: priceCtrl, keyboardType: TextInputType.number, decoration: _input("Prix")),
            const SizedBox(height: 8),
